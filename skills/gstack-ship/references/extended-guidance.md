...
(observed mid-`/gstack-ship`: `script terminated by signal SIGTERM`). Run it through
...
2. **Any UNVERIFIABLE items** (silent gaps — the diff cannot prove them either way). Only fires after NOT DONE is resolved or absent.
   ...
   **Exit conditions:**
   - Any N: STOP. Surface the missing items, suggest re-running /gstack-ship after they're addressed.
...
1. **Classify state** — pure reader, never writes:
   ...
   - **DRIFT_UNEXPECTED** → **STOP**. package.json disagrees with VERSION while VERSION matches base — a manual edit bypassed /gstack-ship. Reconcile manually, then re-run.
...
## Linked Spec
<Auto-detect: look for /gstack-spec archives matching this branch via:
  ...
  # parents — if spec spawned worktree spec/<slug>-$$, the spawned worktree IS where /gstack-ship runs).
...
<If no /gstack-spec archive matches this branch: omit this entire section.>
...
Plan-tune cathedral T15. After a successful ship, surface /gstack-plan-tune once
...
if [ ! -f "$_NUDGE_MARKER" ] && [ "$_QT" = "false" ]; then
  echo ""
  echo "gstack can learn from your AskUserQuestion answers. Run /gstack-plan-tune to opt in"
