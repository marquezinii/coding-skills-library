...
Shared reference for fetching, filtering, and classifying Greptile review comments on GitHub PRs. Both `/gstack-review` (Step 2.5) and `/gstack-ship` (Step 3.75) reference this document.
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || $HOME/.agents/skills/gstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
