...
# /gstack-context-restore — Restore Saved Working Context
...
so without it `/gstack-context-restore` in one worktree could silently load a sibling
...
sibling-worktree save. (`/gstack-context-save list` is the flow that hard-scopes to one
branch.)
...
- `/gstack-context-restore` → load the most recent saved context (any branch)
- `/gstack-context-restore <title-fragment-or-number>` → load a specific saved context
- `/gstack-context-restore list` → tell the user "Use `/gstack-context-save list` — listing
...
if [ ! -d "$CHECKPOINT_DIR" ]; then
  ...
  if [ -z "$ALL" ]; then
    ...
    # `/gstack-context-restore` in worktree A could load worktree B's newer checkpoint.
...
"No saved contexts yet. Run `/gstack-context-save` first to save your current working
state, then `/gstack-context-restore` will find it."
...
- **This is a gstack skill, not a Claude Code built-in.** When the user types
  `/gstack-context-restore`, invoke this skill via the Skill tool.
