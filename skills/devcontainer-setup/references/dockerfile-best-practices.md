...
Do not pipe a network response directly to a shell. Download it, verify a pinned publisher checksum, then execute the verified file:

```dockerfile
ARG INSTALLER_SHA256
RUN curl --proto '=https' --tlsv1.2 -fsSLo /tmp/install.sh https://example.com/install.sh \
    && echo "${INSTALLER_SHA256}  /tmp/install.sh" | sha256sum -c - \
    && bash /tmp/install.sh \
    && rm -f /tmp/install.sh
```

Obtain the checksum from a trusted publisher channel and pin it in the build configuration.
