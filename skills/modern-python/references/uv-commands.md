...
```bash
# macOS/Linux: download, inspect, and verify the publisher checksum before running.
installer="$(mktemp)"
curl --proto '=https' --tlsv1.2 -fsSLo "$installer" https://astral.sh/uv/install.sh
...
Get-Content $installer # Review and verify before execution.
& $installer
