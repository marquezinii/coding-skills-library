...
**If the platform detected above is GitLab or unknown:** STOP with: "GitLab support for /gstack-land-and-deploy is not yet implemented. Run `/gstack-ship` to create the MR, then merge manually via the GitLab web UI." Do not proceed.
...
Before gathering readiness evidence, verify that the VERSION this PR claims is still the next free slot. A sibling workspace may have shipped and landed since `/gstack-ship` ran, leaving this PR's VERSION stale.
...
3. If drift is detected (a PR landed ahead of us and `BRANCH_VERSION < NEXT_SLOT`): **STOP** and print exactly:
   ...
   ⚠ VERSION drift detected.
     ...
     Next free slot:  v<NEXT_SLOT>   (queue moved since last /gstack-ship)

   Rerun /gstack-ship from the feature branch to reconcile. /gstack-ship's ALREADY_BUMPED
   ...
   Exit non-zero. Do NOT auto-bump from `/gstack-land-and-deploy` — rerunning `/gstack-ship` is the clean path (it already handles VERSION + package.json + CHANGELOG header + PR title atomically via Step 12 ALREADY_BUMPED detection).
