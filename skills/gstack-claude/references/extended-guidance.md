...
# /gstack-claude - Claude Outside Voice

You are running the `/gstack-claude` skill from a non-Claude host. This wraps `claude -p`
...
1. `/gstack-claude review` or `/gstack-claude review <instructions>` - **Review mode** (Step 2A)
2. `/gstack-claude challenge` or `/gstack-claude challenge <focus>` - **Challenge mode** (Step 2B)
3. `/gstack-claude` with no arguments, or `/gstack-claude <anything else>` - **Consult mode** (Step 2C)
...
<parsed result from RESP_FILE>
============================================================
Session saved - run /gstack-claude again to continue this conversation.
```
