...
For implementation, see the the tools or connectors available in the current host. Key SMS tools:

| Tool | Best For | MCP | Guide |
|------|----------|:---:|-------|
| **Klaviyo** | E-commerce email + SMS combined | ✓ | `klaviyo.md` (when available) |
| **Postscript** | Shopify DTC SMS, deepest Shopify integration | - | `postscript.md` (when available) |
| **Attentive** | Mid-market+ DTC SMS, full-service | - | `attentive.md` (when available) |
| **Twilio** | Raw API for custom builds, transactional, dev-first | - | `twilio.md` (when available) |
| **Plivo** | Twilio alternative, lower per-send cost | - | `plivo.md` (when available) |
| **AudienceTap** | AI-forward DTC, on-pack QR opt-in | - | `audiencetap.md` (when available) |
| **Brevo** | EU email + SMS, SMB-friendly | ✓ | `brevo.md` (when available) |
| **Customer.io** | Behavior-based SMS automation | - | `customer-io.md` (when available) |
