...
# /gstack-context-save — Save Working Context
...
can resume without losing a beat via `/gstack-context-restore`.
...
- `/gstack-context-save` or `/gstack-context-save <title>` → **Save**
- `/gstack-context-save list` → **List**

If the user provides a title after the command (e.g., `/gstack-context-save auth refactor`),
use it as the title. Otherwise, infer a title from the current work.

If the user types `/gstack-context-save resume` or `/gstack-context-save restore`, tell them:
"Use `/gstack-context-restore` instead — save and restore are separate skills now."
...
Restore later with /gstack-context-restore.
```
...
If the user passes `--all` (e.g., `/gstack-context-save list --all`), show contexts
from **all branches**.
...
`/gstack-context-save` to save your current working state."
...
- **This is a gstack skill, not a Claude Code built-in.** When the user types
  `/gstack-context-save`, invoke this skill via the Skill tool. The old `/checkpoint`
