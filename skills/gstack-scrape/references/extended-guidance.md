...
# /gstack-scrape — pull data from a page
...
2. **Prototype path** (~30s) — no matching skill yet, so drive the page
   with `$B` primitives, return the JSON, and suggest `/gstack-skillify` so the
...
The user's request after `/gstack-scrape` is the intent. If they did not include
one, ask once:
...
> "/gstack-scrape is read-only. For mutating flows, use /automate (browser-skills
...
> "Say /gstack-skillify to make this a permanent skill (200ms on next call)."
...
- Do NOT suggest /gstack-skillify on a broken prototype.
...
- Auth flows / cookie import (use /gstack-setup-browser-cookies first)
...
- Do not embed prose around the JSON in the chat reply unless the user
  asked for an explanation — many `/gstack-scrape` callers pipe the output to
  `jq`.
