...
# /gstack-setup-gbrain — Coding-Agent Onboarding for gbrain
...
## User-invocable
When the user types `/gstack-setup-gbrain`, run this skill. Three shortcut modes:

- `/gstack-setup-gbrain` — full flow (default)
- `/gstack-setup-gbrain --repo` — only flip the per-remote policy for the current repo
- `/gstack-setup-gbrain --switch` — only migrate the engine (PGLite ↔ Supabase)
- `/gstack-setup-gbrain --resume-provision <ref>` — re-enter a previously interrupted
  Supabase auto-provision at the polling step
- `/gstack-setup-gbrain --cleanup-orphans` — list + delete in-flight Supabase projects
...
trap 'echo ""; echo "gstack-gbrain: interrupted. In-flight ref: $INFLIGHT_REF"; \
      echo "Resume: /gstack-setup-gbrain --resume-provision $INFLIGHT_REF"; \
...
a clear "fix and re-run /gstack-setup-gbrain" message. Do NOT continue to Step 5a
...
if ! gbrain init --pglite --json $GBRAIN_EMBED_FLAGS; then
  ...
  echo "Continuing setup without local code search; you can re-run /gstack-setup-gbrain to retry." >&2
fi
```
...
installed). `/gstack-sync-gbrain` will SKIP the code stage cleanly per plan D12.
...
`/gstack-setup-gbrain --switch`. Your original brain is untouched."). STOP.
...
For `/gstack-setup-gbrain --repo` invocations, execute ONLY Step 6 and exit.
...
artifacts (CEO plans, designs, /gstack-investigate reports, retros) rather than
...
```markdown
## GBrain Configuration (configured by /gstack-setup-gbrain)
- Mode: remote-http
- MCP URL: {MCP_URL}
...
Run `/gstack-sync-gbrain` to force-refresh, `/gstack-sync-gbrain --full` for full reindex.

<!-- gstack-gbrain-search-guidance:end -->
```

If Step 9 smoke test fails, skip the guidance block write entirely. The user's
next `/gstack-sync-gbrain` run will re-evaluate capability and write the block when
...
echo "Set up on $(date). Smoke test for /gstack-setup-gbrain." | gbrain put "$SLUG"
...
After Steps 1-9 complete, summarize. Re-running `/gstack-setup-gbrain` on a
...
Re-run `/gstack-setup-gbrain` any time the bearer rotates or the URL moves.
```
...
Run `/gstack-setup-gbrain` again any time gbrain feels off; it's safe and idempotent.
```
...
## `/gstack-setup-gbrain --cleanup-orphans` (D20)
...
- **Concurrent-run lock.** At skill start, `mkdir ~/.gstack/.setup-gbrain.lock.d`
  (atomic). If the mkdir fails, abort with: "Another `/gstack-setup-gbrain` instance
