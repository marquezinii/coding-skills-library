...
# /gstack-benchmark-models — Cross-Model Skill Benchmark

You are running the `/gstack-benchmark-models` workflow. Wraps the `gstack-model-benchmark` binary with an interactive flow that picks a prompt, confirms providers, previews auth, and runs the benchmark.

Different from `/gstack-benchmark` — that skill measures web page performance (Core Web Vitals, load times). This skill measures AI model performance on gstack skills or arbitrary prompts.
...
- **Options:**
  ...
  - B) Only Claude. Completeness: 6/10 (no cross-model signal — use /gstack-ship's review for solo claude benchmarks instead).
