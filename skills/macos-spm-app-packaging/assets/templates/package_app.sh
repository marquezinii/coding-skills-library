...
if [[ ! "$APP_NAME" =~ ^[A-Za-z0-9._-]+$ || "$APP_NAME" == "." || "$APP_NAME" == ".." ]]; then
  echo "APP_NAME must be a safe single path component." >&2
  exit 1
fi
...
[[ "$APP" == "$ROOT/"*.app ]] || { echo "Refusing unsafe APP path: $APP" >&2; exit 1; }
rm -rf -- "$APP"
