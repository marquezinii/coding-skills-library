...
You are running the `/gstack-document-generate` workflow. Your job: produce **high-quality,
...
2. **From /gstack-document-release** — the coverage map identified gaps; you fill them
...
1. Determine what to document:
   ...
   - **If called from /gstack-document-release with gaps**: scope is the specific entities from the coverage map
...
placeholder filter passes obvious docs examples (e.g. `<AWS_ACCESS_KEY_ID>`):
