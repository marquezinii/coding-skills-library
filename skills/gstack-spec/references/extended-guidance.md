...
# /gstack-spec — Author a Backlog-Ready Spec (issue + optional agent spawn)
...
When the user invokes `/gstack-spec`, scan their message for these flags. Flags are space-
...
- **`gh` not authenticated:** print: "Dedupe skipped — `gh auth status` reports
  not logged in. Run `gh auth login` and re-invoke `/gstack-spec` to enable duplicate
...
taxonomy: `lib/redact-patterns.ts` or `/gstack-cso`. Run it on the EXACT spec bytes
...
- **codex not authenticated** (stderr contains "auth"/"login"/"unauthorized"):
  ...
  re-invoke `/gstack-spec`. Continuing to Phase 5." Skip.
- **Timeout (>2 min):** print: "Quality gate skipped — codex didn't respond in
  2 minutes. Skipping ensures `/gstack-spec` stays usable. Run `codex doctor` to
...
The last line records the spec as a durable, issue-scoped cross-session decision so a future session (or `/gstack-ship` closing the issue) inherits the core approach and why, not just the issue link. Non-interactive, best-effort (`|| true`). Substitute `ISSUE_NUMBER` (from the filed issue), `TITLE` (the issue title), and `APPROACH` (the one core approach/decision the spec settled). Only fires when the issue was actually filed.
...
is consumed by `/gstack-ship` for auto-close.
...
The PID suffix and atomic rename prevent collisions when two `/gstack-spec` invocations
...
Capture timestamps at three checkpoints, write to telemetry envelope at /gstack-spec
exit:
...
`/gstack-retro` is a separate follow-up.
...
- **Before `/gstack-spec`:** if the user is still exploring whether to build something,
  route them to `/gstack-office-hours` first. `/gstack-spec` is for work that has already
  passed the "is this worth building" bar.
- **After `/gstack-spec`:** if the spec describes architectural or design risk that
  needs review before implementation starts, suggest `/gstack-plan-eng-review` (or
  `/gstack-autoplan` for the full review gauntlet).
- **For implementation:** the issue itself is the handoff. The implementer can
  open it and execute without re-asking the user.
- **`/gstack-ship` integration:** when `/gstack-ship` opens a PR for a worktree that contains
  a `/gstack-spec` archive (frontmatter `spec_issue_number: <N>`) AND the PR delivers
  the full spec (acceptance criteria checked off per `/gstack-ship`'s existing
  plan-completion gate), `/gstack-ship` adds `Closes #<N>` to the PR body so merging
