...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.url.endsWith('/start')) {
      // Prefer a pinned image with code-server preinstalled. If installation is
      // unavoidable, download to a file and verify a pinned publisher checksum
      // before executing it; never pipe the response directly to a shell.
      await sandbox.exec('curl --proto "=https" --tlsv1.2 -fsSLo /tmp/code-server-install.sh https://code-server.dev/install.sh');
      const expectedDigest = env.CODE_SERVER_INSTALL_SHA256;
      if (!/^[a-f0-9]{64}$/i.test(expectedDigest)) {
        throw new Error('Missing or invalid pinned code-server installer checksum');
      }
      await sandbox.exec(`printf '%s  %s\n' '${expectedDigest}' '/tmp/code-server-install.sh' | sha256sum -c -`);
      await sandbox.exec('sh /tmp/code-server-install.sh');
