...
# /gstack-skillify — codify the last scrape into a permanent skill

The productivity multiplier. `/gstack-scrape` discovered how to pull the data;
`/gstack-skillify` writes it as deterministic Playwright-via-`browse-client`
code so the next `/gstack-scrape` call on the same intent runs in ~200ms.

Without this command, `/gstack-scrape` is a slow wrapper around `$B`. With it,
...
for the most recent `/gstack-scrape` invocation that:
...
> "No recent /gstack-scrape result found in this conversation. Run /gstack-scrape
> <intent> first, then say /gstack-skillify."

Stop. Do not synthesize from chat fragments. Do not synthesize from a
match-path /gstack-scrape result (matched skills are already codified — there's
nothing to skillify).
...
> "The last successful /gstack-scrape was '<intent line>' a few turns back.
> Skillify that one?"
...
- 3–5 trigger phrases the agent should match against in future `/gstack-scrape`
...
```
D<N> — Skill name + tier
Project/branch/task: codifying /gstack-scrape "<intent>" as a browser-skill.
...
The canonical SDK lives at `<gstack-install>/gstack-browse/src/browse-client.ts`.
...
```ts
import { stageSkill } from '<gstack-install>/gstack-browse/src/browser-skill-write';
...
2. If still failing after two retries, OR the failure is an
   ...
   ```ts
   import { discardStaged } from '<gstack-install>/gstack-browse/src/browser-skill-write';
...
Project/branch/task: codified /gstack-scrape "<intent>" — tests pass against fixture.
ELI10: The script ran clean against the snapshot we captured. Saying yes
moves the staged folder into ~/.gstack/browser-skills/ where /gstack-scrape
...
```ts
import { commitSkill } from '<gstack-install>/gstack-browse/src/browser-skill-write';
...
/gstack-scrape calls matching '<canonical-trigger>' will run in ~200ms."
...
- Codify match-path /gstack-scrape results (matched skills are already codified)
- Codify mutating flows (those are /automate's job — Phase 2 P0)
- Run skills (that's `$B skill run` — codified skills are run via /gstack-scrape's
