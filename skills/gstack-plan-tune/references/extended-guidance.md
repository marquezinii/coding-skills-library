...
# /gstack-plan-tune — Question Tuning + Developer Profile (v1 observational)
...
5. If C: do nothing else. Tell the user: "Question tuning stays off. Re-enable
   any time with `/gstack-plan-tune enable` or `gstack-config set question_tuning true`."
...
- Standalone via Step 0's setup gate: `question_tuning` is already `true`
  (user opted in via gstack-config or earlier `/gstack-plan-tune enable`) AND
...
2. Touch the marker so the Setup gate doesn't re-fire:
   ...
   anytime with `/gstack-plan-tune setup` (Step 0 power-user shortcut).

3. Tell the user: "Profile set. Question tuning is on. Use `/gstack-plan-tune`
...
The user has asked to change a preference, either via the `/gstack-plan-tune` menu
...
6. If the user was responding to an inline `tune:` during another skill, note
   ...
   `/gstack-plan-tune` invocations, `source: "plan-tune"` is correct.
...
invokes via `/gstack-plan-tune distill` / `dream`.
...
**When this fires.** The user invokes `/gstack-plan-tune distill` / `dream` /
...
2. If `RATE_CAPPED`: tell the user "You've hit today's 3 distills/day cap.
   Run again tomorrow, or `/gstack-plan-tune stats` for run history."
