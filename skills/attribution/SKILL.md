...
For implementation, see the the tools or connectors available in the current host. Key tools:

| Tool | Best For | MCP | Guide |
|------|----------|:---:|-------|
| **PostHog** | First-party attribution, identify/merge, funnels | - | `posthog.md` (when available) |
| **GA4** | Web analytics, model comparison, user-id stitching | ✓ | `ga4.md` (when available) |
| **Dub** | Short-link + click attribution | ✓ | `dub-co.md` (when available) |
| **Segment** | CDP — route identify/track to every destination | - | `segment.md` (when available) |
| **HubSpot** | CRM lead-source + self-reported fields | ✓ | `hubspot.md` (when available) |
| **Salesforce** | CRM as revenue source of truth | - | `salesforce.md` (when available) |
| **Supermetrics** | Pull platform numbers into one place to reconcile | ✓ | `supermetrics.md` (when available) |
| **RB2B** | De-anonymize B2B website visitors | - | `rb2b.md` (when available) |
