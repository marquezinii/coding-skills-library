  ...
  deploy:
    ...
    steps:
      ...
      - name: Install Aspire CLI
        env:
          ASPIRE_INSTALL_SHA256: ${{ secrets.ASPIRE_INSTALL_SHA256 }}
        run: |
          curl --proto '=https' --tlsv1.2 -fsSLo aspire-install.sh https://aspire.dev/install.sh
          echo "${ASPIRE_INSTALL_SHA256:?missing checksum}  aspire-install.sh" | sha256sum -c -
          bash aspire-install.sh
          rm -f aspire-install.sh
...
steps:
  ...
  - script: |
      curl --proto '=https' --tlsv1.2 -fsSLo aspire-install.sh https://aspire.dev/install.sh
      echo "$(ASPIRE_INSTALL_SHA256)  aspire-install.sh" | sha256sum -c -
      bash aspire-install.sh
      rm -f aspire-install.sh
    displayName: 'Install Aspire CLI'
    env:
      ASPIRE_INSTALL_SHA256: $(ASPIRE_INSTALL_SHA256)
...
```json
{
  ...
  "postCreateCommand": ".devcontainer/install-aspire.sh",
  "forwardPorts": [18888],
  "portsAttributes": {
...
Make `.devcontainer/install-aspire.sh` download the installer to a file, verify a pinned publisher checksum supplied through `ASPIRE_INSTALL_SHA256`, and only then execute it. Never pipe a network response directly to a shell.
