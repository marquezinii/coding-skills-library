...
```bash
# Linux / macOS: download, inspect, and verify the publisher checksum before running.
installer="$(mktemp)"
curl --proto '=https' --tlsv1.2 -fsSLo "$installer" https://aspire.dev/install.sh
...
```powershell
# Windows: download, inspect, and verify the publisher checksum before running.
