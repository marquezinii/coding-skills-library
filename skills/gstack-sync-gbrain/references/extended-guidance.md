...
# /gstack-sync-gbrain — Keep gbrain current and teach the agent to use it

You are running the canonical "keep this brain up to date" verb. /gstack-setup-gbrain
installs gbrain once; /gstack-sync-gbrain runs every time the user wants the brain
...
When the user types `/gstack-sync-gbrain`, run this skill. Argument modes (parsed by
the skill itself, not a dispatcher binary):

- `/gstack-sync-gbrain` — incremental sync (default; mtime fast-path; ~50ms steady-state)
- `/gstack-sync-gbrain --full` — full code reindex via `gbrain reindex-code` (~25-35 min on a big repo). Auto-builds the call graph (`gbrain dream`) **only when it was never built**.
- `/gstack-sync-gbrain --dream` — build this source's call graph (`gbrain code-callers`/`code-callees`) via a source-scoped `gbrain dream --source <id>` cycle; ~minutes; runs lock-free after the sync stages. Always forces, even if already built. Only produces a graph on a code-aware schema pack; otherwise the run reports a WARN explaining why the graph is still empty.
- `/gstack-sync-gbrain --no-dream` — skip the dream cycle that `--full` would otherwise auto-run.
- `/gstack-sync-gbrain --code-only` — only run the code stage; skip memory + brain-sync
- `/gstack-sync-gbrain --dry-run` — preview what would sync; no writes anywhere
- `/gstack-sync-gbrain --no-memory` / `--no-brain-sync` — selectively skip stages
- `/gstack-sync-gbrain --quiet` — suppress per-stage output
- `/gstack-sync-gbrain --refresh-cache` — force-rebuild brain-aware planning cache (v1.48; replaces /brain-refresh-context per D1 fold). Skips code + memory stages; routes to `gstack-brain-cache refresh --project <slug>`.
- `/gstack-sync-gbrain --audit` — emit summary of gstack-owned pages per project + sensitive-content audit (v1.48 / D10 lifecycle). Read-only.
...
Before doing anything, check that /gstack-setup-gbrain has been run on this Mac.
...
Step 9.5 wording in `/gstack-setup-gbrain` (personal vs shared, with persistence
...
> "This repo's gbrain trust policy is `deny`. Run `/gstack-setup-gbrain --repo` to
...
- **`engine-locked`**: STOP. "The local PGLite database is busy, usually
  ...
  or run `/gstack-sync-gbrain` outside the live session, then retry. This identifies
  the conflict but does not remove PGLite's single-process limit."
- **`no-cli`**: STOP. "Local gbrain CLI not installed. Run `/gstack-setup-gbrain`
  first."
- **`missing-config`** AND `gbrain_mcp_mode == "remote-http"`: tell the user
  ...
  symbol code search needs a local PGLite. Run `/gstack-setup-gbrain` and pick
...
- **`missing-config`** AND `gbrain_mcp_mode != "remote-http"`: STOP. "Local
  gbrain CLI is installed but no engine config. Run `/gstack-setup-gbrain` first."
- **`broken-config`** OR **`broken-db`**: STOP with a clear message:
  ...
  engine (status: {gbrain_local_status}). Two options:
    1. Re-run /gstack-setup-gbrain — Step 1.5 offers Retry / Switch to PGLite /
  ...
  Re-run /gstack-sync-gbrain after.
  ```
...
>
> A) Run /gstack-sync-gbrain --full now (recommended)
...
> Note: options differ in kind, not coverage — no completeness score.
>
> A) Run /gstack-sync-gbrain --dream now (recommended)
> B) Skip — I'll run it later
...
Capability check (per /gstack-plan-eng-review §6):
...
```markdown
## GBrain Search Guidance (configured by /gstack-sync-gbrain)
...
built first — run `/gstack-sync-gbrain --dream` (or `--full`) if they return
...
file globs. Run `/gstack-sync-gbrain` after meaningful code changes; for ongoing
auto-sync across all worktrees, run `gbrain autopilot --install` once per
machine — gbrain's daemon handles incremental refresh on a schedule.

Safety: don't run `/gstack-sync-gbrain` while `gbrain autopilot` is active — the
...
search for `## GBrain Search Guidance (configured by /gstack-sync-gbrain)` heading
...
Print a status block matching `/gstack-setup-gbrain` Step 10 conventions. Each row
is `[OK]/[FIX]/[WARN]/[ERR]`. Reuse `gbrain doctor --json --fast` for
informational rows but DO NOT gate the guidance block on doctor (per
/gstack-plan-eng-review §6 — doctor is too strict for unrelated reasons).
...
gbrain status: GREEN
  ...
  ~/.gstack source. OK   <gstack-brain-{user}> (page_count=<N>) — managed by /gstack-setup-gbrain
  Memory sync ..... OK   <artifacts_sync_mode>
  CLAUDE.md ....... OK   ## GBrain Search Guidance present
  Last sync ....... OK   <last_sync from state file>

Run `/gstack-sync-gbrain` again any time gbrain feels off; safe and idempotent.
```
...
2. **Otherwise** fall back to the `CYCLE` value from Step 3.5, with honest wording
   ...
   - `never` → `WARN call graph not built — run /gstack-sync-gbrain --dream`
   - `unknown` → `WARN could not probe call graph (doctor unavailable) — run /gstack-sync-gbrain --dream if code-callers returns 0`
...
check failed; CLAUDE.md guidance block REMOVED — run /gstack-setup-gbrain to repair`).
...
CLAUDE.md but no local gbrain, /gstack-sync-gbrain detects the mismatch via the
...
- **NEEDS_CONTEXT** — /gstack-setup-gbrain has not been run, or `gbrain doctor` shows
