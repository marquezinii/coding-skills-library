...
For implementation, see the the tools or connectors available in the current host. Key prospecting tools:

| Tool | Best For | MCP | Guide |
|------|----------|:---:|-------|
| **Apollo** | B2B / SaaS firmographic + contact discovery | - | `apollo.md` (when available) |
| **Clay** | Multi-source enrichment + waterfall | ✓ | `clay.md` (when available) |
| **Clearbit** | Email-to-company enrichment | - | `clearbit.md` (when available) |
| **ZoomInfo** | Enterprise B2B contact + intent | ✓ | `zoominfo.md` (when available) |
| **Hunter** | Email pattern + verification | - | `hunter.md` (when available) |
| **Snov** | Email finder + verifier | - | `snov.md` (when available) |
| **Truelist** | Email deliverability validation | - | `truelist.md` (when available) |
| **Outreach** | Sales engagement (post-list) | ✓ | `outreach.md` (when available) |
| **RB2B** | Visitor identification (warm intent) | - | `rb2b.md` (when available) |
| **GitHub** | Stargazers/forks/watchers as developer-intent signal | - | `github.md` (when available) |
| **Firecrawl** | Single-target site extraction (prospect's own website) | ✓ | `firecrawl.md` (when available) |
| **Browserbase** | Real-browser site research when rendering or interaction needed | ✓ | `browserbase.md` (when available) |
