...
For implementation, see the the tools or connectors available in the current host. Key RevOps tools:

| Tool | What It Does | Guide |
|------|-------------|-------|
| **HubSpot** | CRM, marketing automation, lead scoring, workflows | `hubspot.md` (when available) |
| **Salesforce** | Enterprise CRM, pipeline management, reporting | `salesforce.md` (when available) |
| **Calendly** | Meeting scheduling, round-robin routing | `calendly.md` (when available) |
| **SavvyCal** | Scheduling with priority-based availability | `savvycal.md` (when available) |
| **Clearbit** | Real-time lead enrichment and scoring | `clearbit.md` (when available) |
| **Apollo** | Contact data, enrichment, and outbound sequences | `apollo.md` (when available) |
| **ActiveCampaign** | Marketing automation for SMBs, lead scoring | `activecampaign.md` (when available) |
| **Zapier** | Cross-tool automation and workflow glue | `zapier.md` (when available) |
| **Introw** | Partner-sourced pipeline, commissions, deal registration, QBRs | `introw.md` (when available) |
| **Crossbeam** | Partner account overlaps and co-sell identification | `crossbeam.md` (when available) |
